...
interface AppContextType {
  ...
  updateDataType: (id: string, dataType: Partial<DataType>) => Promise<DataType>
...
// Exportar o contexto para que possa ser usado como namespace
...
export function AppProvider({ children }: { children: React.ReactNode }) {
  ...
  const updateDataType = async (id: string, dataType: Partial<DataType>) => {
    try {
      ...
      setDataTypes((prev) => prev.map((dt) => (dt.id === id ? formattedDataType : dt)))
      return formattedDataType
    } catch (err: any) {
  ...
  const deleteDataType = async (id: string) => {
    try {
      ...
      toast.success("Tipo de dado excluído", {
        description: "O tipo de dado e seus registros foram excluídos com sucesso.",
      })
    } catch (err: any) {
      console.error("Erro ao excluir tipo de dado:", err)
      setError(err.message || "Erro ao excluir tipo de dado")
      toast.error("Erro ao excluir tipo de dado", {
        description: err.message || "Ocorreu um erro ao excluir o tipo de dado.",
      })
      throw err
    } finally {
      setIsLoading(false)
  ...
  const updateEntry = async (id: string, entry: Partial<Entry>) => {
    try {
      ...
      toast.error("Erro ao atualizar registro", {
        description: err.message || "Ocorreu um erro ao atualizar o registro.",
      })
  ...
  const deleteEntry = async (id: string) => {
    try {
      ...
      toast.error("Erro ao excluir registro", {
        description: err.message || "Ocorreu um erro ao excluir o registro.",
      })
