...
export function DataTypeDialog({ open, onOpenChange, editId }: DataTypeDialogProps) {
  ...
  const [fields, setFields] = useState<Field[]>([{ id: crypto.randomUUID(), name: "", type: "text", required: false }])
  ...
  const resetForm = () => {
    ...
    setFrequency("daily")
    setColor("#3b82f6")
    setFields([{ id: crypto.randomUUID(), name: "", type: "text", required: false }])
    setError("")
  }

  const addField = () => {
    setFields([...fields, { id: crypto.randomUUID(), name: "", type: "text", required: false }])
  }
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-[90vh] md:max-w-4xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle>{isEditing ? "Editar Tipo de Dado" : "Novo Tipo de Dado"}</DialogTitle>
          <DialogDescription>
            {isEditing
                ...
                {fields.map((field) => (
                  <div key={field.id} className="grid grid-cols-1 md:grid-cols-5 gap-4 p-4 border rounded-md">
                    <div className="col-span-2">
                      <Label htmlFor={`field-name-${field.id}`}>Nome</Label>
                      <Input
                    ...
                    <div className="flex items-center gap-2">
                      <input
                        ...
                        className="h-4 w-4"
                      />
                      <Label htmlFor={`field-required-${field.id}`}>Obrigatório</Label>
                    </div>
